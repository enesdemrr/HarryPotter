...
class HomeRepositoryImpl @Inject constructor(private val homeDataSource: HomeDataSource) :
    HomeApiRepository,
    BaseRepository() {
    override suspend fun getAllCharacter(): Flow<NetworkWrapper<List<GetAllCharacterResponseModelEntity>>> {

        return safeApiCall { homeDataSource.getAllCharacter() }.mapResponseToEntity { responseList ->
            responseList.map{
                it.getAllCharacterToEntity()
            }
        }
    }
}