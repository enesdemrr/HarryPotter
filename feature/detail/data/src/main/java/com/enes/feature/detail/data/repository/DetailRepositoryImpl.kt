...
import com.enes.feature.detail.data.mapper.getCharacterDetailToEntity
...
class DetailRepositoryImpl @Inject constructor(private val detailDataSource: DetailDataSource) :
    ...
    override suspend fun getCharacterDetail(id: String): Flow<NetworkWrapper<List<GetCharacterDetailResponseModelEntity>>> {
        return safeApiCall { detailDataSource.getCharacterDetail(id) }.mapResponseToEntity { list ->
            list.map {
                it.getCharacterDetailToEntity()
            }
        }
    }
}